...
HeyNYC saves conversation text so the assistant can understand follow-up questions and avoid appearing to forget an ongoing conversation. In-progress application drafts may also be saved so you can return and finish them. Public hosted deployments [require encryption and run a scheduled deletion process](../heynyc/channels/app.py) for local conversation and draft files after the configured retention period, which defaults to 30 days. Pseudonymous operational metrics and [encrypted, pattern-redacted feedback](../heynyc/channels/analytics.py) may be kept longer when needed for security, evaluation, and service improvement.
...
## 4. Service providers and public data sources
...
You can stop SMS messages by replying **STOP**. Reply **HELP** for messaging help. You can also stop using the service at any time.
