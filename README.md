...
- **Prototype, off by default:** the optional benefits application-form draft workflow and translate-at-edge pipeline. On the default Pydantic resident path, high-stakes situation capabilities use a prompted claim-source checker; low-stakes turns skip that extra model call, and the selectable legacy runtime does not use it. The local MiniCheck backend remains optional and is not used on the pilot host. Forms require explicit configuration and encryption settings ([claim-source checker](heynyc/core/nli.py), [runtime integration](heynyc/core/pydantic_runtime/runtime.py)).
...
flowchart TD
    ...
    G --> H["Mechanical checks, plus claim-source checks for high-stakes capabilities"]
...
6. Mechanical checks reject unknown citation IDs, internal markup, discovery-only citations, and exact address, date, money, phone, or unit-number mismatches against structured DATA snapshots. When a high-stakes situation capability participates, a configured claim-source checker also compares each cited claim with its source excerpt. Low-stakes turns skip that extra model call. If support still fails, HeyNYC preserves the useful text and source link and explains the limitation. High-stakes web claims require authoritative evidence; a directly supporting editorial excerpt may support a low-stakes event claim ([search trust grading](heynyc/core/tools/web_search.py), [grounding guard](heynyc/core/grounding.py), [runtime validator](heynyc/core/pydantic_runtime/runtime.py), [safety boundaries](SAFETY.md)).
...
**How do I know it isn't making things up?** HeyNYC distinguishes sourced claims from material it could not confirm. Before delivery, mechanical checks catch unknown citations and exact structured values that disagree with their cited records. On the default Pydantic path, high-stakes situation capabilities add a claim-source check; ordinary low-stakes searches avoid that extra model call. If support still fails, the resident sees the useful text, an honest limitation, and the relevant source links instead of a generic failure message. Live evaluations review the complete behavior. HeyNYC also never decides eligibility or submits anything on your behalf; the full design is in [SAFETY.md](SAFETY.md).
...
**What would this cost a city?** Built-in telemetry records model, tool, latency, and cost usage for each evaluated turn. High-stakes situation capabilities include a claim-source checker call, while low-stakes turns skip it. Deployments can measure that cost and latency separately from answer generation and compare model choices against the same live evaluation gate ([SAFETY.md](SAFETY.md), [telemetry implementation](heynyc/core/telemetry.py)).
