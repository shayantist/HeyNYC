...
_Last updated: 2026-08-12 (runtime verification and source-discovery behavior clarified)._
...
On top of that floor is a **mechanical citation check** (we call it Part C). The runtime rejects unknown citation IDs, discovery-only evidence, internal markup, and exact address, date, money, phone, or unit-number values that disagree with the cited structured snapshot. The comparison uses the record captured at query time rather than a re-fetch ([runtime validator](heynyc/core/pydantic_runtime/runtime.py), [grounding implementation](heynyc/core/grounding.py)).
...
- **Feedback-and-retry.** On a failure, the agent receives the specific offending claim and reason, then gets one chance to regenerate the complete answer. The [Pydantic runtime](heynyc/core/pydantic_runtime/runtime.py) does not prune paragraphs or splice together a partially accepted answer. This is external, concrete feedback, the attribute-and-revise pattern that works precisely because the correction signal comes from outside the model[^rarr], not open-ended "are you sure?" self-reflection, which research shows can make accuracy worse without an external signal[^self-correct].
- **Abstain.** If the complete answer still fails, it does not ship. Abstaining is the cheaper error[^abstention]: for our users a confident wrong answer is the expensive one.
...
This is not a mathematical truth guarantee. The guard checks whether the captured evidence supports the generated claim. Source freshness, authority, and completeness remain separate responsibilities.
...
- **Grade sources after discovery.** Live [web search](heynyc/core/tools/web_search.py) retains unlisted results instead of treating a hand-maintained domain list as complete. Known domains provide ranking and trust metadata, not an acquisition allowlist. High-stakes claims still require authoritative evidence captured by a grounded tool, while lower-trust results can help discover the primary source. Retrieved text remains an untrusted input because poisoned retrieval is a documented attack surface.[^poisonedrag]
