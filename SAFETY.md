...
**A failed mechanical check does not erase the work already done.** The runtime keeps useful text and retrieved records, asks the answer model to correct the exact mismatch, and retains the relevant source links. If correction fails, it preserves the material and explains the specific limitation instead of replacing the answer with generic copy ([runtime boundary](heynyc/core/pydantic_runtime/runtime.py)). This directly addresses the failure pattern documented in reporting on New York City's MyCity chatbot, including guidance that businesses could take workers' tips or refuse tenants using housing vouchers ([The Markup and THE CITY](https://themarkup.org/artificial-intelligence/2024/03/29/nycs-ai-chatbot-tells-businesses-to-break-the-law)).
...
The default configured Pydantic resident path always applies mechanical checks. The experimental model-based claim-source checker does not run on resident turns. It remains available to explicit calibration and A/B evaluation commands until it clears a human-labeled calibration gate ([runtime selection](README.md#current-status), [configured runtime](heynyc/core/pydantic_runtime/__init__.py), [calibration method](heynyc/eval/README.md#claim-source-checker-calibration)).
...
- **Evaluation-only claim-source check.** An explicit calibration or A/B run may compare cited claims with their source excerpts. The prompted checker produced false rejections of supported city records and honest limitations, so its verdict cannot alter resident output until measured against human labels with an acceptable false-rejection rate ([claim-source checker](heynyc/core/nli.py), [calibration method](heynyc/eval/README.md#claim-source-checker-calibration)).

The claim-source checker is an evaluation candidate, not an oracle. It judges whether retrieved text supports a claim, not whether the source is correct, current, or complete. Complete traces are reviewed during live evaluations, including the resident request, tool calls, retrieved evidence, answer, and citations ([evaluation conventions](heynyc/eval/README.md)).
