...
_Last updated: 2026-08-16 (high-stakes citation ownership clarified)._

HeyNYC helps New Yorkers find, understand, and apply for government services. It answers questions about benefits, housing, food, and immigration, the places where a confident wrong answer can cost someone money, their home, or their immigration status. So the bar here isn't "usually right." The bar for those high-stakes claims is: **every fact is grounded in authoritative evidence, from a sufficient direct excerpt or fetched page, and cited, or the assistant says it doesn't know.** For an explicitly low-stakes capability, an unverified search excerpt may support only the exact claim it states, with its source still identified as unverified ([search trust grading](heynyc/core/tools/web_search.py), [runtime validation](heynyc/core/pydantic_runtime/runtime.py)). This doc is how we try to hit that bar and, just as importantly, where we still fall short. It's written to be read by a skeptic.
...
- **Feedback-and-retry.** On a failure, the agent receives the specific offending claim and reason, then gets up to two bounded chances to regenerate the complete answer. The [Pydantic runtime](heynyc/core/pydantic_runtime/runtime.py) does not prune paragraphs or splice together a partially accepted answer. This is external, concrete feedback, the attribute-and-revise pattern that works precisely because the correction signal comes from outside the model[^rarr], not open-ended "are you sure?" self-reflection, which research shows can make accuracy worse without an external signal[^self-correct].
- **Abstain or warn.** If the complete answer still contains an exact contradiction, internal markup, an unregistered URL, or moderated content, it does not ship. When the only exhausted failure is discovery-grade evidence, the runtime preserves the candidate, appends a specific note naming that source, and keeps the turn in an error state rather than calling it verified ([runtime validator](heynyc/core/pydantic_runtime/runtime.py)). Abstaining is the cheaper error[^abstention]: for our users a confident wrong answer is the expensive one.
